...
export const journal = async (
  ...
  const data = await customFetch(
    `journals?from=${args.from}&to=${args.to}`,
    apiKey,
  );
...
export const journalSummary = async (
  ...
  const data = await customFetch(
    `journals/summary?from=${args.from}&to=${args.to}`,
    apiKey,
  );
...
export const trade = async (
  ...
  const data = await customFetch(
    `trades?from=${args.from}&to=${args.to}`,
    apiKey,
  );
...
export const tradeSummary = async (
  ...
  const data = await customFetch(
    `trades/summary?from=${args.from}&to=${args.to}`,
    apiKey,
  );
...
export const watchlistDetail = async (
  args: WatchlistIdArgs,
  context: Context<SessionData>,
): Promise<HandlerReturnType> => {
  const apiKey = context.session?.apiKey as string;

  const data = await customFetch(`watchlists/${args.id}`, apiKey);

  return formatResponse(data);
};

export const tradeSummaryChart = async (
  args: TradeSummaryChartArgs,
  context: Context<SessionData>,
): Promise<HandlerReturnType> => {
  const apiKey = context.session?.apiKey as string;

  const data = await customFetch(
    `trades/summary-chart?from=${args.from}&to=${args.to}`,
    apiKey,
  );
